...
final class AutomatorWP_HappyForms_Integration {
    ...
    public function admin_notices() {
        ...
        if ( ! $this->meets_requirements() && ! defined( 'AUTOMATORWP_ADMIN_NOTICES' ) ) : ?>
            ...
            <div id="message" class="notice notice-error is-dismissible">
                <p>
                    <?php printf(
                        __( 'AutomatorWP - HappyForms requires %s and %s in order to work. Please install and activate them.', 'automatorwp-happyforms-integration' ),
                        '<a href="https://wordpress.org/plugins/automatorwp/" target="_blank">AutomatorWP</a>',
                        '<a href="https://wordpress.org/plugins/happyforms/" target="_blank">HappyForms</a>'
                    ); ?>
                </p>
            </div>
        ...
        <?php elseif ( $this->pro_installed() && ! defined( 'AUTOMATORWP_ADMIN_NOTICES' ) ) : ?>
            ...
            <div id="message" class="notice notice-error is-dismissible">
                <p>
                    <?php echo __( 'You can uninstall AutomatorWP - HappyForms Integration because you already have the pro version installed and includes all the features of the free version.', 'automatorwp-happyforms-integration' ); ?>
                </p>
            </div>
    ...
    public function load_textdomain() {
        ...
        $lang_dir = AUTOMATORWP_HAPPYFORMS_DIR . '/languages/';
        $lang_dir = apply_filters( 'automatorwp_happyforms_languages_directory', $lang_dir );

        // Traditional WordPress plugin locale filter
        $locale = apply_filters( 'plugin_locale', get_locale(), 'automatorwp-happyforms-integration' );
        $mofile = sprintf( '%1$s-%2$s.mo', 'automatorwp-happyforms-integration', $locale );

        // Setup paths to current locale file
        $mofile_local   = $lang_dir . $mofile;
        $mofile_global  = WP_LANG_DIR . '/automatorwp-happyforms-integration/' . $mofile;

        if( file_exists( $mofile_global ) ) {
            // Look in global /wp-content/languages/automatorwp-happyforms-integration/ folder
            load_textdomain( 'automatorwp-happyforms-integration', $mofile_global );
        } elseif( file_exists( $mofile_local ) ) {
            // Look in local /wp-content/plugins/automatorwp-happyforms-integration/languages/ folder
            load_textdomain( 'automatorwp-happyforms-integration', $mofile_local );
        } else {
            // Load the default language files
            load_plugin_textdomain( 'automatorwp-happyforms-integration', false, $lang_dir );
        }
